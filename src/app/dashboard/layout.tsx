...
import { Image as ImageIcon, History, LogOut, Menu, Settings, User, Feather } from 'lucide-react'; 
...
const navItems = [
  { href: '/dashboard', label: 'Dashboard', icon: Feather }, 
...
export default function DashboardLayout({
  ...
  const NavLinks = () => (
    navItems.map((item) => (
      ...
      >
        <Link href={item.href}>
          <item.icon className={`mr-3 h-5 w-5 ${pathname === item.href ? 'text-secondary-foreground' : 'text-primary'}`} /> 
          {item.label}
        </Link>
      </Button>
    ))
